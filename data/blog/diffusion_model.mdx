...
![Banner](/static/images/blog/diffusion_model/banner.png)
...
- [Weng (2021): Diffusion Models Blog](https://lilianweng.github.io/posts/2021-07-11-diffusion-models/)
...
We just introduced ELBO, following we explain how a VAE is trained to maximize it. In VAEs, the encoder is modeled as a Gaussian distribution parameterized by $\phi$, while the prior distribution over the latent variable is chosen to be a standard Gaussian:
...
In the continuous-time limit, diffusion can be written as a stochastic differential equation (SDE), and the reverse process (together with a deterministic _probability-flow_ ODE) links DDPM- and DDIM-style sampling to score-based dynamics.
