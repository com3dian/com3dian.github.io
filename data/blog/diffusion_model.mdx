...
From Equation (Eq. `eq:gaussian`), we have the expression for $x_0$. After training a DDPM model, a neural network can be used to approximate the noise component $\epsilon_t$ with $\hat{\epsilon}_\theta(x_t,t)$. Therefore, both the original and estimated forms of $x_0$ can be written as
...
the sampling process reduces to the original DDPM formulation (Appendix `app:ddim-ddpm-equiv`).
