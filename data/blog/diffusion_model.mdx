...
However, this reverse conditional is generally intractable to compute directly. Therefore, diffusion models instead learn a parameterized model
$
p_\theta(x_{t-1}\mid x_t)
$ to approximate the reverse diffusion process.
...
This suggests that, instead of directly predicting the posterior mean, we can train a neural network
$
\hat{\epsilon}_\theta(x_t,t)
$
to predict the noise added at timestep $t$.
...
After training, generation proceeds through the reverse diffusion process. We first sample
$
x_T \sim \mathcal{N}(0,I),
$ which corresponds to pure Gaussian noise.
...
From Equation (\ref{eq:gaussian}), we have the expression for $x_0$. After training a DDPM model, a neural network can be used to approximate the noise component $\epsilon_t$ with $\hat{\epsilon}_\theta(x_t,t)$. Therefore, both the original and estimated forms of $x_0$ can be written as
...
Substituting the estimate of $x_0$ into this expression gives
...
This defines the DDIM sampling process:
...
We can treat $\sigma_t^2$ as a hyperparameter that controls the stochasticity of the sampling process. When $\sigma_t^2 = 0$, the sampling process becomes fully deterministic. When
...
the sampling process reduces to the original DDPM formulation (Appendix \ref{app:ddim-ddpm-equiv}).

During generation, it is not necessary to follow the entire sequence of timesteps $\{1,\dots,T\}$. Instead, we may sample using only a subset of timesteps. Let $m < n$ denote two timesteps in this trajectory. The DDIM reverse update step is then given by
...
2. Since sampling does not need to follow the full sequence of timesteps, generation can be performed using only a subset
